 ...
 * @author LAPTOP
 */
@WebServlet(name = "ReadChapterServlet", urlPatterns = {"/ReadChapterServlet"})
public class ReadChapterServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Chapter ch = chapterDAO.getChapterById(chapterId);
            ...
            // Cấu hình whitelist (cho phép HTML an toàn)
            Safelist safelist = Safelist.relaxed()
                    .addAttributes("img", "src", "alt", "title")
                    .addProtocols("img", "src", "http", "https");

            // Làm sạch nội dung chapter
            String cleanHtml = Jsoup.clean(ch.getContent(), safelist);

            // Đưa dữ liệu sang JSP
            request.setAttribute("chapter", ch);
            request.setAttribute("contentHtml", cleanHtml);
